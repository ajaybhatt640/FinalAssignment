
data = {

    "doctorListing": [

        {

            "docName": "Shashank",

            "doctorID": 100,

            "specialization": "Dental",

            "qualification": "BDS",

            "rating": 5,

            "experience": 7,

            "cost": 500,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Khetrapal Nursing Home",

                    "addressLineTwo": "Gn 5, F N g Sector Rajouri Garden",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.7,

                    "lng": 77.8



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },

        {

            "docName": "Deeksha",

            "doctorID": 200,

            "specialization": "Cardiologists‎ ",

            "qualification": "MD",

            "rating": 5,

            "experience": 10,

            "cost": 5000,

            "gender": "female",

            "address": [

                {

                    "addressLineOne": "Lifecare Nursing Home",

                    "addressLineTwo": "Dwarka",

                    "city": "New Delhi",

                    "state": "Delhi",



                    "lat": 28.5732921,


                    "lng": 76.9771919



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },











        {

            "docName": "Aman",

            "doctorID": 101,

            "specialization": "Dental",

            "qualification": "MBBS",

            "rating": 4,

            "experience": 5,

            "cost": 2000,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "AIIMS",

                    "addressLineTwo": "Ansari Nagar East, Gautam Nagar, Ansari Nagar East",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5658898,


                    "lng": 77.2089529



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Vivek",

            "doctorID": 102,

            "specialization": "Geriatricians‎",

            "qualification": "BDS",

            "rating": 3,

            "experience": 6,

            "cost": 1200,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "DermaClinix-The complete Skin and Hair Solution Center",

                    "addressLineTwo": "E-13, Basement, Defence Colony, Near Andrews Ganj Bus Stop, Block E, Defence Colony",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.566763,


                    "lng": 77.2270273



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },





        {

            "docName": "Karan",

            "doctorID": 103,

            "specialization": "Gastroenterologists‎",

            "qualification": "MD",

            "rating": 2,

            "experience": 5,

            "cost": 500,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Child Health Care Center",

                    "addressLineTwo": "C-23, Kharera, Hauz Khas",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5499813,


                    "lng": 77.1720887



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Shivli",

            "doctorID": 104,

            "specialization": "Allergologists‎",

            "qualification": "DO",

            "rating": 5,

            "experience": 1,

            "cost": 100,

            "gender": "Female",

            "address": [

                {

                    "addressLineOne": "Orthopaedic Clinic",

                    "addressLineTwo": "C1 Ln, Block C, Vasant Vihar",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5702367,


                    "lng": 77.1597194



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Charu",

            "doctorID": 105,

            "specialization": "Dental",

            "qualification": "BDS",

            "rating": 4,

            "experience": 3,

            "cost": 500,

            "gender": "female",

            "address": [

                {

                    "addressLineOne": "Dental OPD, AIIMS",

                    "addressLineTwo": "Center for Dental Education and Research, AIIMS Campus, Ansari Nagar East, Block D2, Ansari Nagar East",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5657211,

                    "lng": 77.2108789


                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Ajay",

            "doctorID": 106,

            "specialization": "Diabetologists‎ ",

            "qualification": "MBBS",

            "rating": 2,

            "experience": 9,

            "cost": 10000,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Surgeine Healthcare",

                    "addressLineTwo": "AB-4, 2nd Floor, Shalimar Bagh, AA Block, Poorbi Shalimar Bag, Shalimar Bagh",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.70707,

                    "lng": 28.70707



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Vaibhav",

            "doctorID": 107,

            "specialization": "Cardiologists‎",

            "qualification": "MBBS",

            "rating": 5,

            "experience": 5,

            "cost": 400,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Khasra no 840, janki max lane, Mandi Rd, Dera Village, Mandi, New Delhi, Delhi 110047",

                    "addressLineTwo": "Janki Max Hospital",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.4531632,

                    "lng": 77.0704706

                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Pranjal",

            "doctorID": 108,

            "specialization": "Internists‎",

            "qualification": "DO",

            "rating": 5,

            "experience": 0.5,

            "cost": 100,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Indian Spinal Injuries Centre, Sector C Vasant Kunj, Opposite vasant Valley School, Delhi, IAA Colony, Sector D, Vasant Kunj, New Delhi, Delhi 110070",

                    "addressLineTwo": "Dr Rajat Mahajan- Best Spine Surgeon in Delhi, Spine Specialist,Scoliosis Doctor,Neck Pain Treatment",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": "28.4531632 ",

                    "lng": "77.0704706"





                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Yash",

            "doctorID": 109,

            "specialization": "Geriatricians‎",

            "qualification": "BDS",

            "rating": 5,

            "experience": 15,

            "cost": 5000,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "AB-4, 2nd Floor, Shalimar Bagh, AA Block, Poorbi Shalimar Bag, Shalimar Bagh",

                    "addressLineTwo": "Surgeine Healthcare",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": "  28.70707",

                    "lng": "77.1663503"





                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Vyapak",

            "doctorID": 109,

            "specialization": "Internists‎",

            "qualification": "BMBS",

            "rating": 2,

            "experience": 2,

            "cost": 300,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Center for Dental Education and Research, AIIMS Campus, Ansari Nagar East, Block D2, Ansari Nagar Eas",

                    "addressLineTwo": "Dental OPD, AIIMS",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": " 28.5657211",

                    "lng": "77.2108789"





                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Rohit",

            "doctorID": 110,

            "specialization": "Heart",

            "qualification": "BDS",

            "rating": 2,

            "experience": 6,

            "cost": 50,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Orthopaedic Clinic",

                    "addressLineTwo": "C1 Ln, Block C, Vasant Vihar",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": " 28.5702367",

                    "lng": "77.1597194"




                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Soumya",

            "doctorID": 100,

            "specialization": "Diabetologists‎ ",

            "qualification": "MD",

            "rating": 5,

            "experience": 3,

            "cost": 500,

            "gender": "female",

            "address": [

                {

                    "addressLineOne": "Child Health Care Center",

                    "addressLineTwo": "C-23, Kharera, Hauz Khas",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": "28.5499813",

                    "lng": "77.1720887",





                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Varun",

            "doctorID": 111,

            "specialization": "Internists‎ ",

            "qualification": "MBBS",

            "rating": 2,

            "experience": 3,

            "cost": 300,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "DermaClinix-The complete Skin and Hair Solution Center",

                    "addressLineTwo": "E-13, Basement, Defence Colony, Near Andrews Ganj Bus Stop, Block E, Defence Colony",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": "28.566763",

                    "lng": "77.2270273",





                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Shubhra",

            "doctorID": 113,

            "specialization": "Immunologists‎",

            "qualification": "BDS",

            "rating": 3,

            "experience": 20,

            "cost": 10000,

            "gender": "female",

            "address": [

                {

                    "addressLineOne": "Lifecare Nursing Home",

                    "addressLineTwo": "Dwarka",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": "28.5732921",

                    "lng": "76.9771919"



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Aakash",

            "doctorID": 114,

            "specialization": "Hygienists‎ ",

            "qualification": "MD",

            "rating": 5,

            "experience": 5,

            "cost": 3000,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Khetrapal Nursing Home",

                    "addressLineTwo": "Gn 5, F N g Sector Rajouri Garden",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.7,

                    "lng": 77.8



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Bonzi",

            "doctorID": 115,

            "specialization": "Cardiologist",

            "qualification": "BMBS",

            "rating": 5,

            "experience": 15,

            "cost": 7000,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Lifecare Nursing Home",

                    "addressLineTwo": "Dwarka",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5732921,

                    "lng": 76.9771919



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Amulya",

            "doctorID": 116,

            "specialization": "Diabetologists‎ ",

            "qualification": "MD",

            "rating": 5,

            "experience": 20,

            "cost": 10000,

            "gender": "female",

            "address": [

                {

                    "addressLineOne": "Lifecare Nursing Home",

                    "addressLineTwo": "Dwarka",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5732921,

                    "lng": 76.9771919



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Mark",

            "doctorID": 117,

            "specialization": "Gynaecologists‎",

            "qualification": "MBBS",

            "rating": 2,

            "experience": 3,

            "cost": 200,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "Lifecare Nursing Home",

                    "addressLineTwo": "Dwarka",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5732921,

                    "lng": 76.9771919



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Jim",

            "doctorID": 118,

            "specialization": "Hematologists‎ ",

            "qualification": "MD",

            "rating": 5,

            "experience": 8,

            "cost": 1200,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "AIIMS",

                    "addressLineTwo": "Ansari Nagar East, Gautam Nagar, Ansari Nagar East",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5658898,

                    "lng": 77.2089529



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Kiran",

            "doctorID": 119,

            "specialization": "Leprologists‎ ",

            "qualification": "MBBS",

            "rating": 5,

            "experience": 8,

            "cost": 1200,

            "gender": "female",

            "address": [

                {

                    "addressLineOne": "AIIMS",

                    "addressLineTwo": "Ansari Nagar East, Gautam Nagar, Ansari Nagar East",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5658898,

                    "lng": 77.2089529




                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        },



        {

            "docName": "Lavish",

            "doctorID": 120,

            "specialization": "Hygienists‎ ",

            "qualification": "BMBS",

            "rating": 5,

            "experience": 12,

            "cost": 1500,

            "gender": "male",

            "address": [

                {

                    "addressLineOne": "AIIMS",

                    "addressLineTwo": "Ansari Nagar East, Gautam Nagar, Ansari Nagar East",

                    "city": "New Delhi",

                    "state": "Delhi",

                    "lat": 28.5658898,

                    "lng": 77.2089529



                }

            ],

            "slots": {

                "slotInterval": 15,

                "morning": [

                    {

                        "startTime": "08:00",

                        "endTime": "10:00"

                    }

                ],

                "afternoon": [

                    {

                        "startTime": "10:00",

                        "endTime": "15:00"

                    }

                ],

                "evening": [

                    {

                        "startTime": "15:00",

                        "endTime": "18:00"

                    }

                ],

                "night": [

                    {

                        "startTime": "18:00",

                        "endTime": "22:00"

                    }

                ]

            }

        }

    ]

}
$(document).ready(function () {

    data.doctorListing.forEach(item => {
        var index = 0;

        $('#container').append(`<div class="row Card">

	<div class="col-md-3">

	<div class="Pic">

              <img src="fdoc.jpg">

            </div>

	</div>

	<div class="col-md-9">

	<div class="row Details">

	<div class="col-md-6 DetailsColI">

	<p class="Name">`+ item.docName + `</p>

	<p class="Qualification">`+ item.qualification + `</p>

	<p class="Specialisation">`+ item.specialization + ` ( ` + item.experience + ` years )</p>

	</div>

	<div class="col-md-6 DetailsColII">

	<div class="Address">

	<div><i class="fas fa-map-marker-alt"></i></div>

	<div>

	<p>`+ item.address[index].addressLineOne + `</p>

	<p>`+ item.address[index].addressLineTwo + `</p>

	<p>`+ item.address[index].city + ` | ` + item.address[index].state + `</p>

	</div>

	</div>

	<p class="Cost"><i class="fas fa-rupee-sign"></i>`+ item.cost + `</p>

	</div>

	</div>

	</div>

	<div class="Book">

	<a>Book</a>

	</div>

	</div>`

        )

        index++;

    })

});
$(document).ready(function () {
 $("button").click(function () {
 var x = document.getElementById("dvMap");
 x.style.display = "none";
 var y = document.getElementById("bookingDiv");
 y.style.display = "block";
 });
});


